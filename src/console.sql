...
select round(avg(year_of_issue)) as avg_year from cars;
...
select round(sum(price)) as Mercedes_Benz_price_sum from cars where brand = 'Mercedes-Benz';

--#13 Отобразите самую дорогую  и самую дешевую машину
select brand, model, price from cars where price = (select max(price) from cars) or price = (select min(price) from cars);
...
select * from cars where price in (select price from cars order by price desc limit 10);

--#16 Отобразите 10 новейших машин с 5-го по 15-е.
select brand, model, year_of_issue from (select brand, model, year_of_issue from cars order by year_of_issue desc limit 10 offset 4);
